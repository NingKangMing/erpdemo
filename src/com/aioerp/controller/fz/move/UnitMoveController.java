  ...
  extends BaseController
{
  public void index()
  {
    ...
    setAttr("supId", supId);
    setAttr("pageMap", Unit.dao.getPageBySupId(loginConfigName(), ALL_PRIVS, ALL_PRIVS, ALL_PRIVS, pageNum, numPerPage, supId.intValue(), null, "", null, null));
    render("page.html");
  }
  ...
  public void sourceList()
  {
    ...
    if (upObjectId > 0)
    {
      setAttr("pageMap", Unit.dao.getPageByUpObject(configName, ALL_PRIVS, ALL_PRIVS, ALL_PRIVS, listID, supId.intValue(), upObjectId, pageNum, numPerPage, null, null, null, "rank", "asc", AioConstants.STATUS_DELETE));
      setAttr("objectId", Integer.valueOf(upObjectId));
    }
    else
    {
      setAttr("pageMap", Unit.dao.getPageBySupId(configName, ALL_PRIVS, ALL_PRIVS, ALL_PRIVS, pageNum, numPerPage, supId.intValue(), null, listID, null, null));
    }
    int pSupId = 0;
    if (supId.intValue() > 0) {
      pSupId = Unit.dao.getPsupId(configName, supId.intValue());
    }
    setAttr("supId", supId);
  ...
  public void move()
  {
    ...
    String configName = loginConfigName();
    Unit aimObj = (Unit)Unit.dao.findById(configName, aimId);
    ...
    for (int i = 0; i < ss.length; i++)
    {
      ...
      int sSupId = Unit.dao.getPsupId(configName, id.intValue());
      if (supId == sSupId)
      {
    ...
    {
      boolean verify = Unit.dao.verify(configName, "unitId", aimId);
      if (verify)
      {
    ...
    try
    {
      ...
      for (int i = 0; i < ss.length; i++)
      {
        ...
        Unit p = (Unit)Unit.dao.findById(configName, Integer.valueOf(id));
        String pPids = p.getStr("pids");
        p.set("supId", Integer.valueOf(supId));
        p.set("pids", pids + "{" + id + "}");
        p.update(configName);
        List<Model> ms = Unit.dao.getListByPids(configName, pPids + "{");
        for (Model m : ms)
        {
          Unit sp = (Unit)m;
