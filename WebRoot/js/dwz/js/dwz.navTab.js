...
var navTab = {
	...
	_contextmenu1:function($obj){ // navTab右键菜单
		var $this = this;
		$obj.contextMenu('navTabCM1', {
			bindings:{
				closeCurrent:function(t,m){
					var tabId = t.attr("tabid");
					if (tabId) $this.closeTab(tabId);
	...
	_switchTab: function(iTabIndex){
		...
		this._scrollCurrent();
		this._reload($tab);
		$("div.j-resizeGrid:visible").each(function(){
	...
	_closeTab: function(index, openTabid, isVerify){
		var _this = this;
		var verify = true;
		if(isVerify==false)verify=isVerify;
		//关闭前校验
		var navTab = this._getPanels().eq(index);
		var whichCallBack = $("#whichCallBack",navTab).val();
		if(verify==true && whichCallBack){
			var _input = navTab.find("tbody").find("input");
			if(_input.length>0){
				alertMsg.confirm(("您修改的单据未保存，确定关闭？"), {
					okCall: function(){
						//关闭
						_this._getTabs().eq(index).remove();
						_this._getPanels().eq(index).trigger(DWZ.eventType.pageClear).remove();
						_this._getMoreLi().eq(index).remove();
						if (_this._currentIndex >= index) _this._currentIndex--;
						if (openTabid) {
							var openIndex = _this._indexTabId(openTabid);
							if (openIndex > 0) _this._currentIndex = openIndex;
						}
						_this._init();
						_this._scrollCurrent();
						_this._reload(_this._getTabs().eq(_this._currentIndex));
					},
					cancelCall: function(){
	...
	closeCurrentTab: function(openTabid,isVerify){ //openTabid 可以为空，默认关闭当前tab后，打开最后一个tab
		if (this._currentIndex > 0) {this._closeTab(this._currentIndex, openTabid, isVerify);}
	},
	...
	_loadUrlCallback: function($panel){
		var firstInp = navTab.getCurrentPanel().find("input[type='text']").not("[readonly='readonly']").first();
		if(firstInp.attr("onblur")==""){
			var v = firstInp.val();
	...
	_reload: function($tab, flag){
		...
		if (flag && url) {
			...
			if ($tab.hasClass("external")){
				...
				//获取pagerForm参数
				var $pagerForm = $("form", $panel);
				var args = $pagerForm.size()>0 ? $pagerForm.serializeArray() : {}
				if(args.length&&args.length>0){
					var item={}; 
					item['name'] = "whichRefresh"; 
					item['value'] ="navTabRefresh"; 
					args.push(item);
				}
				$panel.loadUrl(url, args, function(){navTab._loadUrlCallback($panel);},"POST");
			}
		}
	},
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		...
		var op = $.extend({title:"New Tab", data:options.data, fresh:true, external:false}, options);
		...
		if (iOpenIndex >= 0){
			...
			if(op.fresh || $tab.attr("url") != url) {
				...
				if (op.external || url.isExternalUrl()) {
					...
					$panel.ajaxUrl({
						type:method, url:url, data:op.data, callback:function(){
							navTab._loadUrlCallback($panel);
						}
					});
				}
			...
			if(op.freshBtn!=undefined&&op.freshBtn==false){
				this._contextmenu1($tabs.filter(":last").hoverClass("hover"));
			}else{
