				...
				// Create a reference to the jQuery DOM object
					...
					var settings = $.extend( {
						...
						buttonClass : '', // A class name to add to the browse
											// button DOM object
						buttonCursor : 'hand', // The cursor to use with the
												// browse button
						buttonImage : null, // (String or null) The path to an
											// image to use for the Flash browse
											// button if not using CSS to style
											// the button
						buttonText : 'SELECT FILES', // The text to use for
														// the browse button
						...
						fileSizeLimit : 0, // The maximum size of an uploadable
											// file in KB (Accepts units B KB MB
											// GB if string, 0 for no limit)
						fileTypeDesc : 'All Files', // The description for file
													// types in the browse
													// dialog
						fileTypeExts : '*.*', // Allowed extensions in the
												// browse dialog (server-side
												// validation should also be
												// used)
						height : 30, // The height of the browse button
						itemTemplate : false, // The template for the file
												// item in the queue
						method : 'post', // The method to use when sending
											// files to the server-side upload
											// script
						multi : true, // Allow multiple file selection in the
										// browse dialog
						formData : {}, // An object with additional data to
										// send to the server-side upload script
										// with every file upload
						preventCaching : true, // Adds a random value to the
												// Flash URL to prevent caching
												// of it (conflicts with
												// existing parameters)
						progressData : 'percentage', // ('percentage' or
														// 'speed') Data to show
														// in the queue item
														// during a file upload
						queueID : false, // The ID of the DOM object to use
											// as a file queue (without the #)
						queueSizeLimit : 999, // The maximum number of files
												// that can be in the queue at
												// one time
						removeCompleted : true, // Remove queue items from the
												// queue when they are done
												// uploading
						removeTimeout : 3, // The delay in seconds before
											// removing a queue item if
											// removeCompleted is set to true
						requeueErrors : false, // Keep errored files in the
												// queue and keep trying to
												// upload them
						successTimeout : 30, // The number of seconds to wait
												// for Flash to detect the
												// server's response after the
												// file has finished uploading
						uploadLimit : 0, // The maximum number of files you
											// can upload
					...
					// (Array) A list of default event handlers to skip
							/*
							 * onCancel // Triggered when a file is cancelled
							 * from the queue onClearQueue // Triggered during
							 * the 'clear queue' method onDestroy // Triggered
							 * when the uploadify object is destroyed
							 * onDialogClose // Triggered when the browse dialog
							 * is closed onDialogOpen // Triggered when the
							 * browse dialog is opened onDisable // Triggered
							 * when the browse button gets disabled onEnable //
							 * Triggered when the browse button gets enabled
							 * onFallback // Triggered is Flash is not detected
							 * onInit // Triggered when Uploadify is initialized
							 * onQueueComplete // Triggered when all files in
							 * the queue have been uploaded onSelectError //
							 * Triggered when an error occurs while selecting a
							 * file (file size, queue size limit, etc.) onSelect //
							 * Triggered for each file that is selected
							 * onSWFReady // Triggered when the SWF button is
							 * loaded onUploadComplete // Triggered when a file
							 * upload completes (success or error) onUploadError //
							 * Triggered when a file upload returns an error
							 * onUploadSuccess // Triggered when a file is
							 * uploaded successfully onUploadProgress //
							 * Triggered every time a file progress is updated
							 * onUploadStart // Triggered immediately before a
							 * file upload starts
							 */
							}, options);
					...
					if (flashInstalled) {
						...
						$button
								.html(
										'<span class="uploadify-button-text">' + settings.buttonText + '</span>')
								.css( {
									'height' : settings.height + 'px',
						...
						swfuploadify.queueData = {
							...
							filesErrored : 0, // The number of files that
												...
												// select operation
							uploadsSuccessful : 0, // The number of files that
													// were successfully
													// uploaded
							...
							queueLength : 0, // The number of files in the
												// queue
							queueSize : 0, // The size in bytes of the entire
											// queue
							uploadSize : 0, // The size in bytes of the upload
											// queue
							queueBytesUploaded : 0, // The size in bytes that
						...
						if (args[0] == '*') {
							...
							for ( var n = 0; n < args.length; n++) {
								...
								$('#' + args[n]).find('.data').removeClass(
										'data').html(' - Cancelled');
								$('#' + args[n])
										.find('.uploadify-progress-bar')
										.remove();
								$('#' + args[n]).delay(1000 + 100 * n).fadeOut(
										500, function() {
											$(this).remove();
										});
							}
