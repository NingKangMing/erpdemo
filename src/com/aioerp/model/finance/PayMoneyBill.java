...
public class PayMoneyBill
  extends BaseDbModel
{
  public static final PayMoneyBill dao = new PayMoneyBill();
  public static final String TABLE_NAME = "cw_paymoney_bill";
  ...
  {
    return Db.use(configName).queryLong("select count(*) from cw_paymoney_bill where code =? and id != ?", new Object[] { code, Integer.valueOf(id) }).longValue() > 0L;
  }
  ...
  public List<Record> getOrderByUnitLook(String configName, int cwBillTypeId, int cwBillId, int unitId)
  {
    ...
    sb.append("  from cw_pay_by_order c left join b_unit x on x.id=c.unitId where c.unitId=" + unitId + " and c.billTypeId=" + AioConstants.BILL_ROW_TYPE_2 + " and c.cwBillTypeId=" + cwBillTypeId + " and c.cwBillId=" + cwBillId);
    sb.append(" union all ");
  ...
  public List<Record> getOrderByUnit(String configName, int unitId)
  {
    ...
    sb.append("(IFNULL(x.taxMoneys,0)-IFNULL(x.privilege,0)-IFNULL(x.payMoney,0)-IFNULL((select sum(settlementAmount) from cw_pay_by_order where unitId=" + unitId + " and billTypeId=" + AioConstants.BILL_ROW_TYPE5 + " and billId=x.id),0) ) lastMoney,");
    sb.append("(select sum(settlementAmount) from cw_pay_by_order where unitId=" + unitId + " and billTypeId=" + AioConstants.BILL_ROW_TYPE5 + " and billId=x.id) settlementAmount");
    ...
    sb.append("select " + AioConstants.BILL_ROW_TYPE6 + " as billTypeId,'进货退货详情' navTabTitle,x.id,x.code,x.recodeDate,x.remark,x.taxMoneys*-1,x.privilege*-1,x.payMoney*-1,");
    sb.append("(IFNULL(x.taxMoneys*-1,0)-IFNULL(x.privilege*-1,0)-IFNULL(x.payMoney*-1,0)-IFNULL((SELECT SUM(settlementAmount) FROM cw_pay_by_order WHERE unitId=" + unitId + " AND billTypeId=" + AioConstants.BILL_ROW_TYPE6 + " AND billId=x.id),0)  ) lastMoney,");
    sb.append("(select sum(settlementAmount) from cw_pay_by_order where unitId=" + unitId + " and billTypeId=" + AioConstants.BILL_ROW_TYPE6 + " and billId=x.id) settlementAmount");
    ...
    sb.append("(IFNULL(x.gapMoney,0)--IFNULL(x.privilege,0)-IFNULL(x.payMoney,0)-IFNULL((SELECT SUM(settlementAmount) FROM cw_pay_by_order WHERE unitId=" + unitId + " AND billTypeId=" + AioConstants.BILL_ROW_TYPE12 + " AND billId=x.id),0)  ) lastMoney,");
    sb.append("(select sum(settlementAmount) from cw_pay_by_order where unitId=" + unitId + " and billTypeId=" + AioConstants.BILL_ROW_TYPE12 + " and billId=x.id) settlementAmount");
  ...
  {
    Record r = Db.use(configName).findFirst("select sum(settlementAmount) settlementAmount from cw_pay_by_order where unitId=" + unitId + " and cwBillTypeId=" + cwBillTypeId + " and cwBillId=" + cwBillId);
  ...
  {
    Db.use(configName).update("delete from cw_pay_by_order where unitId=" + unitId + " and cwBillTypeId=" + cwBillTypeId + " and cwBillId=" + cwBillId);
  }
}
