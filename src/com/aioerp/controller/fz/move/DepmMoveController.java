...
public class DepmMoveController
  extends BaseController
{
  public void index()
  {
    ...
    setAttr("supId", supId);
    setAttr("pageMap", Department.dao.getPageBySupId(loginConfigName(), pageNum, numPerPage, supId.intValue(), "", "rank", "asc", ALL_PRIVS));
    render("page.html");
  }
  ...
  public void sourceList()
  {
    ...
    if (upObjectId > 0)
    {
      setAttr("pageMap", Department.dao.getSupPageBySupId(configName, pageNum, numPerPage, supId.intValue(), upObjectId, listID, "rank", "asc", ALL_PRIVS));
      setAttr("objectId", Integer.valueOf(upObjectId));
    }
    else
    {
      setAttr("pageMap", Department.dao.getPageBySupId(configName, pageNum, numPerPage, supId.intValue(), "", "rank", "asc", ALL_PRIVS));
    }
    int pSupId = 0;
    if (supId.intValue() > 0) {
      pSupId = Department.dao.getPsupId(configName, supId.intValue());
    }
    setAttr("supId", supId);
  ...
  public void move()
  {
    ...
    String configName = loginConfigName();
    Department aimObj = (Department)Department.dao.findById(configName, aimId);
    ...
    for (int i = 0; i < ss.length; i++)
    {
      ...
      int sSupId = Department.dao.getPsupId(configName, id.intValue());
      if (supId == sSupId)
      {
    ...
    {
      boolean verify = Department.dao.verify(configName, "departmentId", aimId);
      if (verify)
      {
    ...
    try
    {
      ...
      for (int i = 0; i < ss.length; i++)
      {
        ...
        Department p = (Department)Department.dao.findById(configName, Integer.valueOf(id));
        String pPids = p.getStr("pids");
        p.set("supId", Integer.valueOf(supId));
        p.set("pids", pids + "{" + id + "}");
        p.update(configName);
        List<Model> ms = Department.dao.getListByPids(configName, pPids + "{");
        for (Model m : ms)
        {
          Department sp = (Department)m;
