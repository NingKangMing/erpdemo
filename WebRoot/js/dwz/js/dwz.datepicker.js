	...
	$.fn.datepicker = function(opts){
		...
		return this.each(function(){
				...
				$days.not('.disabled').click(function(){
					...
					if (!dp.hasTime()) {
						...
						if($this.hasClass("stealth")){
							...
							var trP = $this.parents("tr:eq(0)");
							var cname = $this.attr("cname");
							divP.text(dp.formatDate(dp.changeDay($day.attr("day"), $day.attr("chMonth"))));
							trP.find("input[cname='"+cname+"']").val(dp.formatDate(dp.changeDay($day.attr("day"), $day.attr("chMonth"))));
						}
						closeCalendar(); 
					} else {
			...
			$this.click(function(event){
				closeCalendar();
				var hasClear = $this.attr("hasClear");
				var dp = new Datepicker($this.val(), opts);
				var offset = $this.offset();
				var z = $this.closest('.dialog').css("zIndex");//从自己一层一层往外匹配父节点
				var iTop = offset.top+this.offsetHeight;
				if(hasClear&&hasClear=="no"){
					$(DWZ.frag['calendarNoClearFrag']).appendTo("body").css({
						zIndex:z,
						left:offset.left+'px',
						top:iTop+'px'
				...
				$(setting.clearBut$).click(function(){
					...
					if($this.hasClass("stealth")){
						var cname = $this.attr("cname");
						var trP = $this.parents("tr:eq(0)");
						trP.find("input[cname='"+cname+"']").val("");
						$this.remove();
					}
					closeCalendar();
				});
				$(setting.okBut$).click(function(){
					...
					if (dp.hasTime()) {
						if(!$.isNumeric(parseInt($(setting.hour$).val())))$(setting.hour$).val("00");
						if(!$.isNumeric(parseInt($(setting.minute$).val())))$(setting.minute$).val("00");
						if(!$.isNumeric(parseInt($(setting.second$).val())))$(setting.second$).val("00");
					...
					if($this.hasClass("stealth")){
						...
						var trP = $this.parents("tr:eq(0)");
						var cname = $this.attr("cname");
						divP.text(dp.formatDate(date));
						trP.find("input[cname='"+cname+"']").val(dp.formatDate(date));
					}
					closeCalendar();
				});
