...
(function($){
	...
	$.fn.extend({
		...
		loadUrl: function(url,data,callback,type,async){
			$(this).ajaxUrl({url:url, data:data, callback:callback,type:type,async:async});
		},
		initUI: function(){
		...
		layoutH: function($refBox){
			return this.each(function(){
				...
				var iH = iRefH - iLayoutH > 65 ? iRefH - iLayoutH : 65;
				
				if ($this.isTag("table")) {
					$this.removeAttr("layoutH").wrap('<div layoutH="'+iLayoutH+'" style="overflow:auto;height:'+iH+'px"></div>');
				} else {
					if($this.attr("class") && "gridTbody2"==$this.attr("class")){
						$this.height(iH).css("overflow-y","auto");
					}else{
