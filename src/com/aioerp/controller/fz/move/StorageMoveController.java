  ...
  extends BaseController
{
  public void index()
  {
    ...
    setAttr("supId", supId);
    setAttr("pageMap", Storage.dao.getPageBySupId(loginConfigName(), "", pageNum, numPerPage, supId.intValue(), "rank", "asc", ALL_PRIVS));
    render("page.html");
  }
  ...
  public void sourceList()
  {
    ...
    if (upObjectId > 0)
    {
      setAttr("pageMap", Storage.dao.getSupPage(configName, pageNum, numPerPage, supId.intValue(), upObjectId, listID, "rank", "asc", ALL_PRIVS));
      setAttr("objectId", Integer.valueOf(upObjectId));
    }
    else
    {
      setAttr("pageMap", Storage.dao.getPageBySupId(configName, listID, pageNum, numPerPage, supId.intValue(), "rank", "asc", ALL_PRIVS));
    }
    int pSupId = 0;
    if (supId.intValue() > 0) {
      pSupId = Storage.getPsupId(configName, supId.intValue());
    }
    setAttr("supId", supId);
  ...
  public void move()
  {
    ...
    String configName = loginConfigName();
    Storage aimObj = (Storage)Storage.dao.findById(configName, aimId);
    ...
    for (int i = 0; i < ss.length; i++)
    {
      ...
      int sSupId = Storage.getPsupId(configName, id.intValue());
      if (supId == sSupId)
      {
    ...
    {
      boolean verify = Storage.dao.verify(configName, "storageId", aimId);
      if (verify)
      {
    ...
    try
    {
      ...
      for (int i = 0; i < ss.length; i++)
      {
        ...
        Storage p = (Storage)Storage.dao.findById(configName, Integer.valueOf(id));
        String pPids = p.getStr("pids");
        p.set("supId", Integer.valueOf(supId));
        p.set("pids", pids + "{" + id + "}");
        p.update(configName);
        List<Model> ms = Storage.dao.getListByPids(configName, pPids + "{");
        for (Model m : ms)
        {
          Storage sp = (Storage)m;
