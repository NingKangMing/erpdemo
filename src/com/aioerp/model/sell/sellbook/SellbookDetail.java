...
public class SellbookDetail
  extends BaseDbModel
{
  public static final SellbookDetail dao = new SellbookDetail();
  public static final String TABLE_NAME = "xs_sellbook_detail";
  ...
  {
    StringBuffer sql = new StringBuffer("select * from xs_sellbook_detail where billId=" + billId);
    return dao.find(configName, sql.toString());
  }
  
  public List<Model> getList2(String configName, int billId)
  {
    SqlHelper helper = SqlHelper.getHelper();
    helper.addAlias("product", Product.class);
    helper.appendSelect(" select detail.*,product.* ");
    helper.appendSql(" from xs_sellbook_detail as detail");
  ...
  public List<Model> getList(String configName, int billId, String productPrivs)
  {
    SqlHelper helper = SqlHelper.getHelper();
  ...
  {
    StringBuffer sql = new StringBuffer("select * from xs_sellbook_detail where 1=1");
    if (billId != null) {
  ...
  {
    StringBuffer sql = new StringBuffer("delete from xs_sellbook_detail where 1=1");
